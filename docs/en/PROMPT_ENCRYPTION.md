# Prompt Encryption - Protege tus prompts en OpenAI/Anthropic

## ¿Por qué?

Cuando envías prompts a OpenAI o Anthropic, ellos pueden ver exactamente qué escribiste. Con Prompt Encryption, tus prompts están **encriptados end-to-end** - ni siquiera OpenAI/Anthropic puede leerlos.
...
- Usa **AES-256-GCM** (256-bit encryption + autenticación)
...
```env
# Prompt Encryption Key (32 caracteres hex o cualquier string)
...
func main() {
    // Crear encriptador
    encryptor, err := common.NewPromptEncryptor("your-secret-key")
    if err != nil {
        panic(err)
    }
...
- `tag`: 16 bytes de authenticación (incluido en GCM)

### Seguridad
- ✅ **Confidentiality**: AES-256 es militarmente seguro
- ✅ **Integrity**: GCM verifica que no fue manipulado
- ✅ **Authenticity**: Solo quien tenga la key puede desencriptar
...
- `relay/prompt_encryption_middleware.go` - Integración con LLM APIs
...
func main() {
    // Inicializar
    encryptor, err := common.NewPromptEncryptor("my-secret-key-12345")
    if err != nil {
        log.Fatal(err)
    }

    // Prompt original
    original := "How to exploit this vulnerability in PostgreSQL?"
...
**P: ¿Qué pasa si pierdo la master key?**
R: No puedes desencriptar nada. Guarda la key en un lugar seguro (AWS Secrets Manager, HashiCorp Vault, etc.)

**P: ¿OpenAI puede ver mis datos?**
R: OpenAI recibe solo blob encriptado. No pueden verlo sin la key.

**P: ¿Afecta la latencia?**
R: Encriptación AES-256 es muy rápida (~1-2ms). Impacto negligible.

**P: ¿Puedo usar esto con múltiples keys?**
R: Sí, genera múltiples encriptadores con diferentes keys.

**P: ¿Es FIPS 140-2 compliant?**
R: AES-256 sí. Go crypto/aes usa FIPS approved algorithms.

---

## Próximos Pasos

1. Generar tu master key
