...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		// Verb identity token endpoint
		apiRouter.GET("/verb-token", middleware.UserAuth(), controller.GetVerbToken)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.GET("/self/affiliate-token", controller.GetUserAffiliateToken)
				selfRoute.GET("/topup/info", controller.GetTopUpInfo)
				selfRoute.GET("/topup/self", controller.GetUserTopUps)
				selfRoute.POST("/topup", middleware.CriticalRateLimit(), controller.TopUp)
				selfRoute.POST("/pay", middleware.CriticalRateLimit(), controller.RequestEpay)
				selfRoute.POST("/pay/flow", middleware.CriticalRateLimit(), controller.DirectFlowPayment)
				selfRoute.POST("/pay/crypto", middleware.CriticalRateLimit(), controller.DirectCryptoPayment)
